// sw.js — PWA shell cache + auto-update (v3)
const VERSION = 'hub-sw-v3.0.0';
const SHELL = ['./','./index.html','./manifest.webmanifest','./firebase-config.js','./sw.js','./firebase-messaging-sw.js','./icons/icon-192.png','./icons/icon-512.png'];
...
self.addEventListener('fetch', e=>{
  ...
  if(req.mode==='navigate'){ e.respondWith((async()=>{try{const p=await e.preloadResponse; if(p){const c=await caches.open(VERSION); await c.put('./index.html',p.clone()); return p;} const n=await fetch(req); const c=await caches.open(VERSION); await c.put('./index.html',n.clone()); return n;}catch{const c=await caches.open(VERSION); return (await c.match('./index.html')) || new Response('<h1>Offline</h1>',{headers:{'Content-Type':'text/html'}});} })()); return; }
